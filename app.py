...
from xsubtitle.core import UserFacingError, process_job


def run_job(uploaded_path: str | None, model_name: str, progress=gr.Progress()):
    if not uploaded_path:
        raise gr.Error("번역할 영상을 선택해 주세요.")
    try:
        result = process_job(
            uploaded_path,
            model_name,
    ...
    return (
        ...
        str(result.subtitled_video),
        str(result.korean_srt),
        str(result.source_srt),
    )
